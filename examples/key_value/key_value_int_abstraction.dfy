...
module ShardedKVProtocol {
  ...
  function WrongInitialMap(): map<Key, Value> {
    map key | key in AllKeys() :: 1
  }

  predicate Init(c: Constants, v: Variables)
  {
    && v.WF(c)
    && (forall idx:HostIdx | c.ValidHost(idx) :: v.maps[idx] == if idx==0 then WrongInitialMap() else map[])
  }
  ...
  predicate Insert(c: Constants, v: Variables, v': Variables, idx: HostIdx, key: Key, value: Value)
  {
    ...
    && v'.maps == v.maps[idx := v.maps[idx][key := (value + 1)]]
  }
...
/*}*/
  ...
  {
/*{*/
    MapSpec.Variables(map key | key in KnownKeys(c, v) :: (AbstractionOneKey(c, v, key) - 1))
/*}*/
  }
...
/*{*/
  lemma KnownKeysSubsetOfHostKeys(c: Constants, v: Variables)
    requires v.WF(c)
...
/*}*/
  ...
  lemma InsertPreservesInvAndRefines(c: Constants, v: Variables, v': Variables, insertHost: HostIdx, insertedKey: Key, value: Value)
    requires Inv(c, v)
    requires Next(c, v, v') // ???: is it needed
...
/*{*/
    ...
    assert v'.maps[insertHost][insertedKey] == (value + 1);
...
/*{*/
    ...
    // assert key in v.maps[queryHost];
    HostKeysSubsetOfKnownKeys(c, v, queryHost + 1);
    assert key in AllKeys();

    // assert output == v.maps[queryHost][key];
    AnyHostWithKeyIsKeyHolder(c, v, queryHost, key);
    assert AbstractionOneKey(c, v, key) == output + 1;
